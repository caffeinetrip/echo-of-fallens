...
class Enemy(pp.Element):
    ...
    def use_spell(self):
        return self.spells[0].spell.dmg
    ...
    def render(self):
        ...
        if not self.e['Game'].battle_manager.is_battling and not dialogue and self.e['Game'].player.action == 'idle':
            ...
            if self.type == 'main_boss':
                self._handle_boss_prompt_visible(current_time)
            ...
            if self.type == 'main_boss':
                self._handle_boss_prompt_hidden(current_time)
    ...
    def _handle_boss_prompt_visible(self, current_time):
    ...
    def _handle_boss_prompt_hidden(self, current_time):
...
main_boss = Enemy('main_boss', [Spell('dark', 5), Spell('dark', 5), Spell('dark', 5)], 400)
mom_ghost = Enemy('mom_ghost', [Spell('water', 1)], 40)
father_ghost = Enemy('father_ghost', [Spell('water', 1)], 40)